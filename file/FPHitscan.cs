...
namespace UltimateSurvival
{
	...
	public class FPHitscan : FPWeaponBase
	{
        ...
        //new audio for empty click
        [SerializeField]
        [Tooltip("sounds that will play when trying to fire an empty Gun")]
        private SoundPlayer m_EmptyFireAudio;

        //new audio for relload
        [SerializeField]
        [Tooltip("sound that will play when trying to reload")]
        private SoundPlayer m_ReloadAudio;

        //new Audio public variable=reload
        [HideInInspector]
        public SoundPlayer mReloudAudio;

        //new audio public variable = Audiosource
        [HideInInspector]
		...
		protected void Shoot(Camera camera)
		{
            ...
            {
                //play empty fire sound if have no ammo
                m_EmptyFireAudio.Play(ItemSelectionMethod.Randomly, m_AudioSource, 0.3f);
            }
			GameController.Audio.LastGunshot.Set(new Gunshot(transform.position, Player));
		}
        ...
        private void Start()
		{
            ...
            // set the public reload Audio to have the vlue from the private serialised
            if (m_ReloadAudio != null)
                mReloudAudio = m_ReloadAudio;
            // set the public reload Audio to have the vlue from the private serialised
            if (m_AudioSource)
                mAudioSource = m_AudioSource;
